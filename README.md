...
(window-align the SW / chunked KV-tile iteration) and its V2 extension to the
...
* `2` - 2D **and** 3D-segmented base-shift (V2)
...
* Measured 2D-path (V1) kernel latency reduction on a tile-saving step:
  **6.8% (W=128), 4.9% (W=256), 2.0% (W=512)**, tracking `1/⌈W/T⌉`.
* V2 makes the **3D-segmented** path bit-exact across batch shapes (previously
